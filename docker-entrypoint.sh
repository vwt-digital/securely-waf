...
if [[ -v SECURELY ]]; then
  /usr/local/securely-blocker \
    -file /etc/securely-blocker-db \
    -file_post_command "\
      echo 1 > /var/run/securely-blocker-started-apache;
      $* -k graceful $APACHE_ARGUMENTS || true" &

  sleep 5s
  if [ ! -f "/var/run/securely-blocker-started-apache" ]; then
    echo "Securely could not be reached, starting Apache without security configuration"
